...
Generating realistic, but fake, datasets is useful in various settings, from education to software testing, enabling controlled data that precisely fits your needs. There are many tasks you might want to simulate data for, including:

* **Package or app testing**. Fake, but realistic, data is helpful for testing when developing packages or apps.

* **Teaching**. You might need to demonstrate a particular skill or test students in a particular way that requires you to create data that precisely meets your teaching needs.

* **Prototyping a report**. You might need to create a report before the data is available. Simulate data that matches how the final data will look so you can build your report, dashboard, or app ahead of time.

* **Clinical trial simulations**. You might simulate data to explore potential outcomes, estimate statistical power, or plan analyses before conducting an actual clinical trial.
...
Popular large language models (LLMs) are great at simulating data, particularly when accuracy or strict realism isn't required. You might have used the chat interface of ChatGPT or another LLM to simulate data. Some model providers even let you directly download a CSV of the data from the chat interface.

However, if you want to integrate the data directly into your coding environment or automate data generation, using the LLM's API will streamline the process. In this blog post, **we'll show you how interact with LLMs in R with the ellmer package**, creating a command-line tool and a Shiny app that simulate data. You can see the deployed app [here](https://connect.posit.cloud/skaltman/content/01957333-ddf8-765e-de40-f1eba12e6a38). 

![By the end of this blog post, you'll learn how to create a Shiny app that generates a dataset and visualizes it.](images/app.png)
...
As mentioned earlier, common LLMs can simulate data for you, often through a chat interface. Without much prompting, they will generally simulate realistic-looking data that fits a particular description. However, this typically requires downloading the data and manually loading it into R or Python, which adds an extra step to the workflow. 

Let's build a small tool in R using the [ellmer package](https://ellmer.tidyverse.org/) to simulate data. ellmer provides functions for working with LLMs from R. 
...
We don't need to manually specify the API key because dotenv loads our environment variables and `chat_openai()` will look for a variable called `OPENAI_API_KEY`. 
...
First, we need to write a function that **takes a CSV-formatted string and reads it into a dataframe**. We'll also need to **document the function with [roxygen2 comments ](https://roxygen2.r-lib.org/).** These comments help the LLM understand how to use our function, just like they would help a human use your function. 
...
We'll need to make a few changes to the prompt, including telling the app how to use plotting tool. You can see the full prompt [here](https://github.com/skaltman/data-generation-blog/blob/main/prompts/prompt-app.md). 
...
If you know the column names and types of data you want to generate, you could also use [structured data](https://ellmer.tidyverse.org/articles/structured-data.html). The structured data features allows you to define the elements of data you want to extract from text or images. However, this only works if you know the number of columns and their types that you want to extract. 
...
In these examples, we limit the size of the generated datasets to minimize API usage costs. However, you can easily adjust the settings to generate larger datasets as needed.
